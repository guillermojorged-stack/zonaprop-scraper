...
async function scrapeFicha(page, url) {
  ...
  if (process.env.DEBUG_SELECTORS) {
    const debugInfo = await page.evaluate(() => {
      const dataQaEls = [...document.querySelectorAll('[data-qa]')]
        .map((el) => el.getAttribute('data-qa'))
        .filter((v, i, arr) => arr.indexOf(v) === i);
      const priceLike = [...document.querySelectorAll('*')]
        .filter((el) => el.children.length === 0 && /(\$|USD|U\$S)\s?[\d.,]+/.test(el.innerText || ''))
        .slice(0, 5)
        .map((el) => ({ tag: el.tagName, class: el.className, dataQa: el.getAttribute('data-qa'), text: el.innerText }));
      const imgCount = document.querySelectorAll('img').length;
      const imgSamples = [...document.querySelectorAll('img')].slice(0, 5).map((img) => ({
        src: img.getAttribute('src'),
        dataSrc: img.getAttribute('data-src') || img.getAttribute('data-flkty-lazyload'),
        class: img.className,
      }));
      return { dataQaEls, priceLike, imgCount, imgSamples };
    });
    console.log('DEBUG_SELECTORS:', JSON.stringify(debugInfo, null, 2));
  }
