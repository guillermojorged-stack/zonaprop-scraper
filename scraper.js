...
async function main() {
  const urlBusqueda = process.argv[2];
  if (!urlBusqueda) {
    console.error('Uso: node scraper.js "<url-de-busqueda-zonaprop>"');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`Buscando publicaciones en: ${urlBusqueda}`);
    const urls = await scrapeListado(page, urlBusqueda);
